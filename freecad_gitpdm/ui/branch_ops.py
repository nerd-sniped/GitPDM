...
class BranchOperationsHandler:
    ...
    def update_branch_button_states(self):
        ...
        self._parent.switch_branch_btn.setEnabled(
        ...
        current_branch = self._git_client.current_branch(
            self._parent._current_repo_root
        ) if self._parent._current_repo_root else None
        selected_idx = self._parent.branch_combo.currentIndex()
        selected_branch = (
    ...
    def _compute_worktree_path_for_branch(self, branch_name: str) -> str:
        ...
        try:
            base = os.path.basename(os.path.normpath(self._parent._current_repo_root or ""))
            parent = os.path.dirname(os.path.normpath(self._parent._current_repo_root or ""))
            if not parent:
                return ""
    ...
    def _create_and_open_worktree(self, branch_name: str, worktree_path: str):
        ...
        args = [git_cmd, "-C", self._parent._current_repo_root, "worktree", "add", worktree_path, branch_name]
        self._job_runner.run_job(
            "add_worktree",
            args,
