...
def load_config(repo_root: Path) -> FCStdConfig:
    """
    Load configuration from FreeCAD_Automation/config.json or use defaults.
...
def save_config(repo_root: Path, config: FCStdConfig) -> Result:
    """
    Save configuration to FreeCAD_Automation/config.json.
    ...
    try:
        ...
        # Convert to GitCAD format for compatibility
