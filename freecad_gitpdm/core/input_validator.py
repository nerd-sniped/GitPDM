...
def validate_repo_name(name: str) -> Tuple[bool, str]:
    ...
    if not REPO_NAME_PATTERN.match(name):
        return False, "Repository name contains invalid characters (use only letters, numbers, dots, hyphens, underscores)"
...
def validate_owner_name(owner: str) -> Tuple[bool, str]:
    ...
    if not OWNER_NAME_PATTERN.match(owner):
        return False, "Owner name contains invalid characters (use only letters, numbers, hyphens, underscores)"
...
def validate_file_path(file_path: str, repo_root: Optional[str] = None) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        for part in parts:
            if len(part) > MAX_PATH_COMPONENT_LENGTH:
                return False, f"Path component too long (max {MAX_PATH_COMPONENT_LENGTH} chars)"
