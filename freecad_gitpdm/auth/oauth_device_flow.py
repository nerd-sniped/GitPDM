...
def poll_for_token(
    ...
    while True:
        ...
        try:
            ...
            error_body = e.read().decode("utf-8", errors="replace")
            log.error(f"Token poll failed: {e.code} {error_body}")
            raise DeviceFlowError("http_error", f"HTTP {e.code}") from e
        except urllib.error.URLError as e:
            # Transient network error; continue polling
            log.debug(f"Token poll network error (will retry): {e}")
            continue
        except json.JSONDecodeError as e:
        ...
        if "error" in response_data:
            ...
            if error_code == "authorization_pending":
                ...
                log.debug(f"GitHub requested slow_down, new interval: {interval}s")
                continue
            elif error_code == "expired_token":
                log.error("Device code expired")
                raise DeviceFlowError(error_code, error_description)
            elif error_code == "access_denied":
                log.error("User denied GitHub access")
                raise DeviceFlowError(error_code, error_description)
            else:
