...
from freecad.gitpdm.core import log, paths as core_paths
from freecad.gitpdm.export import mapper
...
class FileBrowserHandler:
    ...
    def create_browser_content(self):
        ...
        self._parent.repo_list.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection)
    ...
    def _on_list_files_finished(self, job):
        ...
        # Refresh lock status after file list is updated
        if hasattr(self._parent, '_lock_handler') and self._parent._lock_handler._available:
            self._parent._lock_handler.refresh_lock_status()
    ...
    def _on_list_context_menu(self, pos):
        ...
        selected_items = self._parent.repo_list.selectedItems()
        menu = QtWidgets.QMenu(self._parent)
        
        log.debug(f"Context menu: {len(selected_items)} items selected")
        ...
        # Add configure backups option for single FCStd file selection
        act_backups = None
        rel = item.text() if item else None
        
        # Check if we have FCStd files selected
        fcstd_files = [item.text() for item in selected_items if item.text().lower().endswith(".fcstd")]
        is_multi_select = len(selected_items) > 1
        
        log.debug(f"FCStd files selected: {len(fcstd_files)}, is_multi_select: {is_multi_select}")
        
        if rel and rel.lower().endswith(".fcstd") and not is_multi_select:
            menu.addSeparator()
            act_backups = menu.addAction("Configure Backups...")
        
        # Add lock/unlock actions for FCStd files
        act_lock = None
        act_unlock = None
        act_force_lock = None
        act_lock_bulk = None
        act_unlock_bulk = None
        
        # Sprint 7: Check lock handler availability (read-only, no refresh)
        # check_availability triggers UI updates which close the menu, so we just check the flag
        if fcstd_files and hasattr(self._parent, '_lock_handler') and self._parent._lock_handler._available:
            menu.addSeparator()
            
            if is_multi_select:
                # Bulk operations
                log.debug(f"Adding bulk lock/unlock options for {len(fcstd_files)} files")
                act_lock_bulk = menu.addAction(f"🔒 Lock {len(fcstd_files)} Files")
                act_unlock_bulk = menu.addAction(f"🔓 Unlock {len(fcstd_files)} Files")
            else:
                # Single file operations
                log.debug(f"Adding single file lock/unlock options for {rel}")
                log.debug(f"Current locks in cache: {list(self._parent._lock_handler._current_locks.keys())}")
                lock_info = self._parent._lock_handler.get_file_lock_status(rel)
                log.debug(f"Lock info for '{rel}': {lock_info}")
                
                if lock_info:
                    # File is locked
                    if self._parent._lock_handler.is_locked_by_me(rel):
        ...
        elif fcstd_files and hasattr(self._parent, '_lock_handler'):
            # Lock handler exists but GitPDM not initialized
            # Sprint 7: This should rarely happen due to auto-initialization
            log.debug(f"Lock handler not available, offering manual initialization")
            
            # Try auto-initialization one more time
            if self._parent._current_repo_root:
                from freecad.gitpdm.ui.init_wizard import auto_initialize_if_needed
                was_initialized = auto_initialize_if_needed(self._parent._current_repo_root)
                if was_initialized:
                    log.info(f"Context menu: Auto-initialized GitPDM for repository")
                    # Refresh and rebuild menu
                    self._parent._lock_handler.check_availability(self._parent._current_repo_root)
                    # Don't show init option if we just initialized successfully
                    if self._parent._lock_handler._available:
                        log.debug("Auto-init succeeded, skipping manual init option")
                    else:
                        menu.addSeparator()
                        act_init_gitpdm = menu.addAction("⚙️ Troubleshoot: Reinitialize GitPDM")
                        act_init_gitpdm.setToolTip("Retry initialization if auto-init failed")
                else:
                    # Auto-init was skipped (already initialized?) but still not available
                    log.warning(f"GitPDM config exists but lock handler not available")
                    menu.addSeparator()
                    act_init_gitpdm = menu.addAction("⚙️ Troubleshoot: Reinitialize GitPDM")
                    act_init_gitpdm.setToolTip("Retry initialization if something went wrong")
            else:
                log.warning(f"No repo root set, cannot initialize GitPDM")
        ...
        if chosen == act_open:
        ...
        elif act_lock_bulk and chosen == act_lock_bulk:
            self._parent._lock_handler.lock_files_bulk(fcstd_files, force=False)
        elif act_unlock_bulk and chosen == act_unlock_bulk:
            self._parent._lock_handler.unlock_files_bulk(fcstd_files, force=False)
        elif act_lock and chosen == act_lock:
            self._parent._lock_handler.lock_file(rel, force=False)
        elif act_unlock and chosen == act_unlock:
            self._parent._lock_handler.unlock_file(rel, force=False)
        elif act_force_lock and chosen == act_force_lock:
            # Confirm force lock
            ...
            if reply == QtWidgets.QMessageBox.Yes:
                self._parent._lock_handler.lock_file(rel, force=True)
        elif 'act_init_gitpdm' in locals() and chosen == act_init_gitpdm:
            # Initialize GitPDM
            self._init_gitpdm_from_menu()

    def _init_gitpdm_from_menu(self):
        """Initialize GitPDM in the current repository from context menu."""
        if not self._parent._current_repo_root:
        ...
        success = show_init_wizard(self._parent._current_repo_root, self._parent)
        
        if success:
            # Re-check GitPDM availability
            if hasattr(self._parent, '_lock_handler'):
                ...
                QtWidgets.QMessageBox.information(
                    ...
                    "You can now use file locking features.\n"
                    "Right-click on .FCStd files to lock/unlock them."
                )
                # Refresh file browser to show lock indicators
                self.refresh_files()
            ...
            if json_abs and json_abs.exists():
                try:
                    ...
                    if backup_dir.exists():
                        from freecad.gitpdm.export.backup_manager import (
                            cleanup_old_backups,
                        )
