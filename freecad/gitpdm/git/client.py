...
class GitClient:
    ...
    def commit(self, repo_root, message):
        ...
        from freecad.gitpdm.core import input_validator
    ...
    def get_config(self, repo_root, key, local=False):
        ...
        if local and repo_root:
            # Read only local (repository) config
            args = [git_cmd, "-C", repo_root, "config", "--local", key]
        elif repo_root:
            # Read effective config (local overrides global) from repository
            args = [git_cmd, "-C", repo_root, "config", key]
        else:
            # Read global config
            args = [git_cmd, "config", "--global", key]
